...
void NumberDisplay::post_create()
{
    ...
    if (_digit_textures.size() == 10 && _current == -1)
    {
        set_number(0);
    }
}

void NumberDisplay::set_digit_textures(const std::vector<peng::shared_ref<const rendering::Texture>>& textures)
{
    ...
    {
        digit->set_digit_textures(_digit_textures);
    }
}
